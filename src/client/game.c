...
#include <SDL.h>
static void game_update_delta_time(Game *game) {
    const Uint32 current_time = SDL_GetTicks();
    game->delta_time = (current_time - game->last_time) / 1000.0f; // Convert to seconds
    game->last_time = current_time;
}
static void game_render(const Game *game, SDL_Renderer *renderer, const Player *players, const size_t players_size, const uint16_t player_id) {
...
static uint8_t game_poll_events(Game *game) {
    uint8_t moved = 0;
...
void game_init(Game *game, const uint16_t player_id) {
    // Initialize SDL
