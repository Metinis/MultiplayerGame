...
static void server_send_new_connection(const int sock, struct sockaddr_in *addr, const uint16_t player_id, vec2 pos) {
    Packet packet;
...
static void server_send_handshake(ServerData *server_data, struct sockaddr_in *addr, const uint16_t player_id) {
...
static void server_process_position(ServerData *server_data, vec2 position, const struct sockaddr_in *addr) {
...
static uint8_t is_new_player(const ServerData *server_data, const struct sockaddr_in *cli_addr) {
    for (int i = 0; i < server_data->player_count; i++) {
...
static void add_new_player(ServerData *server_data, struct sockaddr_in *cli_addr) {
...
static void server_process_handshake(ServerData *server_data, char *message, struct sockaddr_in *addr) {
    printf("\nReceived message: %s\n", message);
    if(is_new_player(server_data, addr)) {
        add_new_player(server_data, addr);
    }
}
static void server_receive_packet(ServerData *server_data) {
